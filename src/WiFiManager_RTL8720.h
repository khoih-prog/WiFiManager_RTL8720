...
const char WM_HTML_HEAD_STYLE[] /*PROGMEM*/ =
...
#if USING_BOARD_NAME
  const char WM_HTML_HEAD_END[]   /*PROGMEM*/ =
...
#define BUFFER_LEN      128
    ...
    void saveConfigData()
    {
      ...
      WG_LOGERROR5(F("SaveEEPROM,Sz="), FlashStorage.length(), F(",DataSz="), totalDataSize, F(",WCSum=0x"),
                   String(calChecksum, HEX));
...
#define WIFI_MULTI_CONNECT_WAITING_MS                   500L
    ...
    void createHTML(String& root_html_template)
    {
      ...
      pitem.replace("[[input_id]]",  "<input id='id' list='SSIDs'>"  + String(WM_DATALIST_START) + "'SSIDs'>" + ListOfSSIDs +
                    WM_DATALIST_END);
      WG_LOGDEBUG1(F("pitem:"), pitem);
      pitem.replace("[[input_id1]]", "<input id='id1' list='SSIDs'>" + String(WM_DATALIST_START) + "'SSIDs'>" + ListOfSSIDs +
                    WM_DATALIST_END);
